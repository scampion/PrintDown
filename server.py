...
class SimpleIPPHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            if operation_id == 0x0002:  # Print-Job
                ...
                self._send_ipp_response(0x0501, request_id=request_id)  # server-error-operation-not-supported
    ...
    def _handle_print_job(self, request_data, client_info, request_id):
        ...
        try:
            ...
            if len(document_data) > 0:
                ...
                self._send_ipp_response(0x0000, request_id=request_id)  # successful-ok
            else:
                print("Warning: No document data in Print-Job request")
                self._send_ipp_response(0x0000, request_id=request_id)  # Accept anyway
                
        except Exception as e:
            print(f"Error processing print job: {e}")
            import traceback
            traceback.print_exc()
            self._send_ipp_response(0x0500, request_id=request_id)  # server-error-internal-error
    ...
    def _send_ipp_response(self, status_code, include_printer_attrs=False, request_id=1):
        ...
        response.extend(struct.pack('>I', request_id))  # request-id
        ...
        if include_printer_attrs:
            ...
            for i, op in enumerate(ops):
                if i > 0:
                    response.append(0x23)  # enum (continuation)
                    response.extend(struct.pack('>H', 0))  # empty name
                response.extend(struct.pack('>H', 4))
            ...
            response.append(0x49)  # mimeMediaType
            ...
            self._add_ipp_attribute(response, 0x04, 'document-format-default', 
                                   'application/pdf', 0x49)
            
            # pdl-override-supported
            self._add_ipp_attribute(response, 0x04, 'pdl-override-supported', 
                                   'not-attempted', 0x44)
